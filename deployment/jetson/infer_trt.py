...
# TensorRT major version at which the tensor-name API replaced the binding-index API.
TRT_MAJOR_WITH_TENSOR_API = 10


# ---------------------------------------------------------------- preprocessing
def preprocess(image_path: str | Path, metadata: dict[str, Any]) -> np.ndarray:
    ...
    x = (x - np.asarray(pp["mean"], dtype=np.float32)) / np.asarray(
        pp["std"], dtype=np.float32
    )
...
# ---------------------------------------------------------------- TensorRT glue
def _is_trt10(trt: Any) -> bool:
    """True if the runtime exposes the TensorRT 10 tensor-name API."""
    return int(trt.__version__.split(".")[0]) >= TRT_MAJOR_WITH_TENSOR_API


def load_trt(engine_path: str | Path) -> tuple[Any, Any, Any, Any, str, str, tuple[int, ...]]:
    """Deserialize the engine and return inference handles.

    Returns (trt, cuda, engine, context, input_name, output_name, out_shape).
    ...
    if _is_trt10(trt):
        ...
        out_shape = tuple(
            engine.get_binding_shape(engine.get_binding_index(outputs[0]))
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--engine",
        required=True,
        help="TensorRT engine from build_engine.sh",
    )
    parser.add_argument(
        "--metadata",
        default="metadata.json",
        help="sidecar JSON from export_onnx.py",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="timed iterations for latency/FPS",
    )
    ...
    trt, cuda, _engine, context, input_name, output_name, out_shape = load_trt(
        args.engine
    )

    print(f"engine   : {args.engine}")
    print(f"model    : {metadata['model_arch']} - {metadata['num_classes']} classes")
    print(f"tensorrt : TensorRT {trt.__version__} | arch: {platform.machine()}")
    print()
    ...
    if x.shape != expected:
        raise SystemExit(
            f"input shape mismatch: engine wants {expected}, got {x.shape}"
        )
    ...
    print(f"prediction : {classes[pred]} ({probs[pred] * 100:.2f}% confidence)")
    print("top-5      :")
    for idx in np.argsort(probs)[::-1][:5]:
        print(f"   {probs[idx] * 100:5.2f}%  {classes[int(idx)]}")
    print()
    ...
    print(
        f"latency    : {ms_per_img:.2f} ms/img "
        f"(mean over {args.iters} iters, incl. host<->device copies)"
    )
    print(f"throughput : {1000.0 / ms_per_img:.1f} FPS (batch=1)")
